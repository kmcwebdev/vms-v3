...
export async function GET(req: Request) {
  try {
    ...
    const visitor_count_by_week_in_month_query = `
      -- visitor count by week and year (line chart)
      select to_char(date_trunc('week', visit_time), 'Month DD, YYYY') as formatted_date, count(*) as visitor_count
      from visitor_logs
